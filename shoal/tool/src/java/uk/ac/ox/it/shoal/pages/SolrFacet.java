...
public class SolrFacet<T> extends Panel {
	...
	public static final int FACET_DISPLAY = 5;
	...
	final static Comparator<Count> valueComparator = Comparator.comparing(Count::getName);
	
	final static Comparator<Count> countComparator = (o1, o2) -> Long.signum(o1.getCount() - o2.getCount());
	...
	private class FacetListView extends ListView<Count> {
		...
		private FacetListView(String id, List<Count> list,
		...
		protected void populateItem(ListItem<Count> item) {
			...
			String[] filters = linkParams.getValues("filter").stream().map(StringValue::toString).toArray(String[]::new);
			...
			link.add(new AttributeModifier("title", displayValue));
