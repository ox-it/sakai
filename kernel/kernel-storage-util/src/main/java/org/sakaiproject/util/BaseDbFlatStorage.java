...
public class BaseDbFlatStorage
{
   ...
   /** Our logger. */
	 ...
	 */
	public void writeProperty(final String table, final String idField, final Object id, final String extraIdField, final String extraId,
			final String propName, final String propValue)
	{
		if (table == null || StringUtils.isBlank(propName) || StringUtils.isBlank(propValue)) return;
		...
		// do this in a transaction
		// OWLTODO: when contributing: lambda opportunity
		m_sql.transact(new Runnable()
		{
			public void run()
			{
				writePropertyTx(table, idField, id, extraIdField, extraId, propName, propValue);
			}
		}, "writeProperty:" + id);
	}
