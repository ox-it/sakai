...
package org.sakaiproject.oxford.shortenedurl.servlets;
...
public class ResolverServlet extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		service = (OxfordShortenedUrlService) manager.get(OxfordShortenedUrlService.class);
	}
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 */
		if(parts.length < 2) {
			response.sendError(HttpServletResponse.SC_BAD_REQUEST, "Missing parameter for OxfordShortenedUrlService");
		} else if (parts.length > 2) {
			response.sendError(HttpServletResponse.SC_BAD_REQUEST, "Malformed URL for OxfordShortenedUrlService");
		} else {
