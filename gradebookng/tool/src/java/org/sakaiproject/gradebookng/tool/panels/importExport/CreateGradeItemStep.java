...
public class CreateGradeItemStep extends BasePanel {
	...
	public void onInitialize() {
		...
		final AjaxButton nextButton = new SakaiAjaxButton("nextbutton") {  // OWL
			...
			protected void onSubmit(final AjaxRequestTarget target, final Form<?> form) {
				...
				saveItemAndProceed(true, target, form); // OWL
			}
	...
	private void saveItemAndProceed(boolean forward, final AjaxRequestTarget target, final Form<?> form)
	{
		...
		if (!assignmentNameIsUnique(existingAssignments, newAssignment.getName())
				|| !assignmentNameIsUnique(newAssignment)) {
			validated = false;
		...
		if (validated) {
			...
			processedGradeItem.setAssignmentTitle(newAssignment.getName()); // OWL - need to retain the original title for matching later
			processedGradeItem.setAssignmentPoints(newAssignment.getPoints()); // OWL - doesn't seem like anything actually uses this, but will leave it
