...
public class UpdateUngradedItemsPanel extends Panel {
	...
	public void onInitialize() {
		...
		final GbGradingType gradingType = GbGradingType.valueOf(businessService.getGradebook().getGrade_type());

		// form model
		final GradeOverride override = new GradeOverride();
		override.setGrade(String.valueOf(DEFAULT_GRADE));
		final CompoundPropertyModel<GradeOverride> formModel = new CompoundPropertyModel<>(override);
		...
		if (businessService.getUserRole() == GbRole.TA) {
			boolean categoriesEnabled = businessService.categoriesAreEnabled();
			List<PermissionDefinition> permissions = businessService.getPermissionsForUser(businessService.getCurrentUser().getId());
		...
		final DropDownChoice<GbGroup> groupAndSectionFilter = new DropDownChoice<>(
			"group",
			new PropertyModel<>(override, "group"),
			groups,
		...
		if (!groups.isEmpty()) {
			groupAndSectionFilter.setModelObject((settings.getGroupFilter() != null) ? settings.getGroupFilter() : groups.get(0));
		}
		...
		// confirmation dialog
		add(new Label("confirmationMessage", new StringResourceModel(
