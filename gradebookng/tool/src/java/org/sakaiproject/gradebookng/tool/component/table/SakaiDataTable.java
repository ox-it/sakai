...
public class SakaiDataTable extends AjaxFallbackDefaultDataTable {
	 ...
	 */
	public SakaiDataTable(String id, final List<IColumn> columns, ISortableDataProvider dataProvider, boolean pageable, int pageSize)
	{
		super(id, columns, dataProvider, pageSize);
		((RepeatingView) get("topToolbars:toolbars")).removeAll();
		((RepeatingView) get("bottomToolbars:toolbars")).removeAll();
		
		// OWLTODO: evaluate if this is still even a sakaidatatable without the below components
		/*if(pageable) {
