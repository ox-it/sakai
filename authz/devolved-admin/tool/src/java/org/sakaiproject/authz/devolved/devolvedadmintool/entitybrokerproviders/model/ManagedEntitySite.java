...
public class ManagedEntitySite implements Site {
	...
	private transient List<ManagedEntityGroup> siteGroupsList;
	...
	public ManagedEntitySite() {
	}

	public ManagedEntitySite(String title, String shortDescription, String description, String iconUrl,
	...
	public ManagedEntitySite(String title,
	...
	public ManagedEntitySite(Site site, boolean includeGroups) {
		this.site = site;
		this.id = site.getId();
		...
		if (includeGroups) {
			...
			siteGroupsList = new Vector<ManagedEntityGroup>(groups.size());
			for (Group group : groups) {
				ManagedEntityGroup eg = new ManagedEntityGroup(group);
	 ...
	 */
	public ManagedOwner getSiteOwner() {
		ManagedOwner owner = null;
		if (this.site != null) {
			// TODO handle the contact info?
			User user = site.getCreatedBy();
			owner = new ManagedOwner(user.getId(), user.getDisplayName());
		} else {
			owner = new ManagedOwner(this.owner, this.owner);
		}
		return owner;
	}
	...
	public List<ManagedEntityGroup> getSiteGroups() {
	...
	public List<SitePage> getSitePages() {
		...
		if (site != null) {
			...
			for (SitePage sitePage : pages) {
				...
				if (allowPage) {
					rpgs.add( new ManagedEntitySitePage(sitePage) );
				}
			}
		}
		return rpgs;
	...
	// transient
	public void setSiteGroupsList(List<ManagedEntityGroup> siteGroups) {
