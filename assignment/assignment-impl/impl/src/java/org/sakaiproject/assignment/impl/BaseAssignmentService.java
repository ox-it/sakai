...
//	spring service injection
			...
			IdInvalidException, IdUnusedException
	{
		...
		if (contentReference != null)
		{
			...
			else
			{
				...
				pEdit.removeProperty("lti_id");  // TODO: is this still needed if you have never run the experimental version of the LTI integration?
				addLiveProperties(pEdit);
			}
		}
	...
	// TODO: is this method still needed? Do the callbacks actually still change things in the submission object?
	// If so, refactor to avoid this if possible and remove this method
	@Override
			...
			for (User user : users)
			{
				...
				if (groups != null)
				{
					...
					for (Group _g : groups)
					{
						...
						if (submission != null && releaseToGroupRefs.contains(_g.getReference()) && allowGetSubmission(submission.getReference()))
						{
	...
	public class BaseAssignmentSubmission implements AssignmentSubmission
	{
		...
		private String getReviewError(ContentResource cr)
		{
			...
			try
			{
				...
				ContentReviewItem item = contentReviewService.getFirstItemByContentId(contentId);
				...
				if (status != null)
				{
					...
					else if (ContentReviewItem.NOT_SUBMITTED_CODE.equals(status))
					{
						errorMessage = rb.getFormattedMessage("content_review.notYetSubmitted", new Object[] { contentReviewService.getServiceName() });
					}
					else if (ContentReviewItem.SUBMITTED_REPORT_ON_DUE_DATE_CODE.equals(status))
					{
