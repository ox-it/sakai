...
public class ProviderServlet extends HttpServlet {
    ...
    protected Site findOrCreateSite(Map payload, boolean trustedConsumer) throws LTIException {
        ...
        if (SiteTitleValidationStatus.STRIPPED_TO_EMPTY.equals(status)) {
            M_log.warn("Provided context_title is empty after HTML stripping: {}", context_title_orig);
        } else if (SiteTitleValidationStatus.EMPTY.equals(status)) {
            M_log.warn("Provided context_title is empty after trimming: {}", context_title_orig);
        } else if (SiteTitleValidationStatus.TOO_LONG.equals(status)) {
            M_log.warn("Provided context_title is longer than max site title length of {}: {}", SITE_TITLE_MAX_LENGTH, context_title_orig);
        }
