...
public class OakLdapAttributeMapper extends SimpleLdapCandidateAttributeMapper {
    ...
    public void mapUserDataOntoUserEdit( LdapUserData userData,
        ...
        String displayId = userEdit.getProperties().getProperty(UnboundidDirectoryProvider.DISPLAY_ID_PROPERTY);
        if (displayId == null || displayId.length() == 0) {
            userEdit.getProperties().addProperty(UnboundidDirectoryProvider.DISPLAY_ID_PROPERTY, "none");
        }

        String yearOfStudy = yearOfStudy(userData);
        if (yearOfStudy != null && yearOfStudy.length() > 0) {
            userEdit.getProperties().addProperty(UnboundidDirectoryProvider.YEAR_OF_STUDY_PROPERTY, yearOfStudy);
        ...
        if (primaryOrgUnitDN != null && primaryOrgUnitDN.length() > 0) {
            userEdit.getProperties().addProperty(UnboundidDirectoryProvider.PRIMARY_ORG_UNIT_DN, primaryOrgUnitDN);
        }
    }
